...
train_token_batches, train_gaz_batches, train_label_batches, train_action_batches = pickle.load(f)
f.close()
f = open(config.data_path + "_dev.pkl", 'rb')
dev_token_batches, dev_gaz_batches, dev_label_batches, dev_action_batches = pickle.load(f)
f.close()
f = open(config.data_path + "_test.pkl", 'rb')
test_token_batches, test_gaz_batches, test_label_batches, test_action_batches = pickle.load(f)
f.close()
...
best_model = None
best_per = 0
train_all_batches = list(zip(train_token_batches, train_gaz_batches, train_label_batches, train_action_batches))
...
for e_ in range(config.epoch):
    ...
    batch_counter = 0
    total_loss=0
    for token_batch, gaz_batch, label_batch,  action_batch in train_all_batches:
        #print (action_batch)
        ...
        ner_model.train()
        optimizer.zero_grad()
        loss, (preds, correct_nums, total_nums) = ner_model.forward(token_batch_var,gaz_batch, label_batch, action_batch_var)
        if type(loss) != float:
            total_loss+=loss
            loss.backward()
            ...
            if batch_counter%50==0:
                print("batch {0} with {1} instance and sentece length {2} loss {3}, action acc {4}".format(
                    batch_counter, batch_len, sent_len, total_loss.cpu().data.numpy().item()/50, action_acc))
                total_loss=0
            optimizer.step()
    ...
    cur_time = time.time()
    dev_batch_zip = zip(dev_token_batches, dev_gaz_batches, dev_label_batches, dev_action_batches)
    f1 = get_f1(ner_model, dev_batch_zip, config)
    print("Dev step took {} seconds".format(time.time() - cur_time))

    cur_time = time.time()
    test_batch_zip = zip(test_token_batches,test_gaz_batches, test_label_batches,  test_action_batches)
    test_f1 = get_f1(ner_model, test_batch_zip, config)
